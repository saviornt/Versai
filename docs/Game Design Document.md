...
| **Procedural Universe (Niagara)** | Embeddings = particles, attention = ribbons/fog, gradients = vector storms | Must-have |
...
- **Visual:** Procedural cosmos (Niagara VFX + real-time tessellation). Clean sci-fi/ambient aesthetic — dark nebulae, glowing particle clusters, attention “ribbons”, gradient “storms”.
...
**Kanban Epics / Tasks:**
- [ ] Create full `Versai/` Python package structure (`core.py`, `config.py`, `sonification.py`, `gguf_writer.py`)
- [ ] Implement 4 training styles + custom hook
- [ ] FlexAttention telemetry via `score_mod`
- [ ] `multiprocessing.shared_memory` buffer with loss/embed/attention metrics
- [ ] Basic GGUF auto-save + metadata
- [ ] Simple command-line launch + live loss printing
...
- **Audio**: soundcard (WASAPI Exclusive)
